...
class ProxyHandle(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        :return:
        '''
        if self.path == 'http://baseproxy.ca/' or self.path == 'http://webscan.ca/':
            self._send_ca()
            return
        request = None
        try:
...
class MitmProxy(HTTPServer):
    ...
    def __init__(self, server_addr=('', 8788), request_handler_class=ProxyHandle, bind_and_activate=True, https=True):
        ...
        logger.info('Server is running at address(\'%s\',\'%d\')......' % (server_addr[0], server_addr[1]))
        self.req_plugs = []
