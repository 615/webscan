...
def random_str(length=10, chars=string.digits):
...
def createGithubIssue(errMsg, excMsg):
    ...
    try:
        req = requests.get("https://api.github.com/search/issues?q={}".format(
            quote("repo:w-digital-scanner/webscan Unhandled exception (#{})".format(key))))
    ...
    try:
        r = requests.post("https://api.github.com/repos/w-digital-scanner/webscan/issues", data=json.dumps(data),
                          headers=headers)
    except Exception as e:
        return False
    issueUrl = re.search(r"https://github\.com/w-digital-scanner/webscan/issues/\d+", r.text)
    if issueUrl:
